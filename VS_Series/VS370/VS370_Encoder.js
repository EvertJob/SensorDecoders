...
function milesightDeviceEncode(payload) {
    ...
    if ("idle_report_interval" in payload) {
        encoded = encoded.concat(setIdleReportInterval(payload.idle_report_interval));
    }
    ...
    if ("pir_collection_window_time" in payload) {
        encoded = encoded.concat(setPirCollectionWindowTime(payload.pir_collection_window_time));
    }
    if ("pir_trigger_times_threshold" in payload) {
        encoded = encoded.concat(setPirTriggerTimesThreshold(payload.pir_trigger_times_threshold));
    }
...
/**
 * set idle report interval
 * @param {number} idle_report_interval unit: minute, range: [1, 60]
 * @example { "idle_report_interval": 3 }
 */
function setIdleReportInterval(idle_report_interval) {
    if (typeof idle_report_interval !== "number") {
        throw new Error("idle_report_interval must be a number");
    }
    if (idle_report_interval < 1 || idle_report_interval > 60) {
        throw new Error("idle_report_interval must be in range [1, 60]");
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0x40);
    buffer.writeUInt8(idle_report_interval);
    return buffer.toBytes();
}
...
/**
 * set pir collection window time
 * @param {number} pir_collection_window_time values: (0: "2s", 1: "4s", 2: "6s", 3: "8s")
 * @example { "pir_collection_window_time": 2 }
 */
function setPirCollectionWindowTime(pir_collection_window_time) {
    var pir_collection_window_time_values = [0, 1, 2, 3];
    if (pir_collection_window_time_values.indexOf(pir_collection_window_time) === -1) {
        throw new Error("pir_collection_window_time must be one of " + pir_collection_window_time_values.join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0x42);
    buffer.writeUInt8(pir_collection_window_time);
    return buffer.toBytes();
}

/**
 * set pir trigger times threshold
 * @param {number} pir_trigger_times_threshold values: (0: "1 times", 1: "2 times", 2: "3 times", 3: "4 times")
 * @example { "pir_trigger_times_threshold": 2 }
 */
function setPirTriggerTimesThreshold(pir_trigger_times_threshold) {
    var pir_trigger_times_threshold_values = [0, 1, 2, 3];
    if (pir_trigger_times_threshold_values.indexOf(pir_trigger_times_threshold) === -1) {
        throw new Error("pir_trigger_times_threshold must be one of " + pir_trigger_times_threshold_values.join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0x43);
    buffer.writeUInt8(pir_trigger_times_threshold);
    return buffer.toBytes();
}
